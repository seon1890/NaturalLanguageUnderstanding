...
class TestDataUtils(unittest.TestCase):
    ...
    def test_encode_data(self):
        ...
        input_ids, attention_masks = data_utils.encode_data(self.dataset, self.tokenizer, self.max_seq_len)
        test = torch.Size((len(self.dataset), self.max_seq_len))
        
        self.assertEqual(input_ids.shape , test)
        self.assertEqual(attention_masks.shape , test)
        self.assertEqual(type(input_ids), torch.Tensor) 
        self.assertEqual(type(attention_masks), torch.Tensor)
